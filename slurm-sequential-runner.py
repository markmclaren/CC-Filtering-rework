...
def submit_and_wait_for_job(
    ...
    n_files,
    template_content,
    job_name_prefix="batch_job",
    partition=None,
    time_limit=None,
    mem=None,
    cpus_per_task=None,
    check_interval=60,
    segments_per_task=100,
    throttle=50,
    dry_run=False,
):
    """
    ...
    while not job_completed:
        try:
            ...
            if str(job_id) not in result.stdout:
                ...
                print(
                    f"Job {job_id} for date {date} still running. Checking again in {check_interval} seconds..."
                )
...
def process_crawl_data_sequentially(
    crawl_dates,
    n_files_list,
    template_content,
    job_name_prefix="batch_job",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Submit sequential SLURM jobs with template file"
    )
    parser.add_argument(
        "--template-file", required=True, help="Path to command template file"
    )
    ...
    parser.add_argument(
        "--crawl-dates-file", help="Path to file with crawl dates and file counts"
    )
    parser.add_argument(
        "--segments-per-task", type=int, default=100, help="Segments per task"
    )
    parser.add_argument(
        "--throttle",
        type=int,
        default=50,
        help="Maximum number of concurrent array tasks",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run in dry-run mode (do not submit jobs)",
    )
