...
if command -v squeue >/dev/null 2>&1; then
    RUNNING=$(squeue -u $USER -j "*${JOB_PREFIX}*" --format="%i %P %j %T %l %D" --state=RUNNING | wc -l)
    PENDING=$(squeue -u $USER -j "*${JOB_PREFIX}*" --format="%i %P %j %T %l %D" --state=PENDING | wc -l)
    COMPLETED=$(sacct -u $USER -j "*${JOB_PREFIX}*" --format="JobID,JobName,State,Elapsed,MaxRSS" --state=COMPLETED | tail -n +2 | wc -l)
    ...
    squeue -u $USER -j "*${JOB_PREFIX}*" --format="%i %P %j %T %l %M %D %R" --states=RUNNING,PENDING || echo "No matching jobs found."
else
...
if [[ $REMAINING_SEGMENTS -eq 0 ]]; then
    ...
    if [[ -d "$LOG_DIR" ]]; then
        RECENT_LOGS=$(ls -t "$LOG_DIR"/*.log | head -5)  # Last 5 logs
...
TIME_LIMIT=$(squeue -u $USER -j "*${JOB_PREFIX}*" --format="%l" --states=RUNNING | head -n2 | tail -1 | cut -d: -f1)  # e.g., "14-00:00:00" -> hours
if [[ -n "$TIME_LIMIT" ]]; then
    TIME_HRS=${TIME_LIMIT%%:*}  # Extract hours
    ELAPSED=$(squeue -u $USER -j "*${JOB_PREFIX}*" --format="%D" --states=RUNNING | head -n2 | tail -1 | cut -d: -f1)  # Days elapsed
...
echo "-----------------------------"
FREE_GB=$(df -BG . | tail -1 | awk '{print $4}' | sed 's/G//')  # Free space in GB
