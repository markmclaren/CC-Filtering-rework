...
DRY_RUN="${2:-}"
ANALYSIS_DIR="${ANALYSIS_DIR:-.}"  # Dir to scan; use --output-dir=DIR to override
...
# Override ANALYSIS_DIR if --output-dir provided (parse args simply)
for arg in "$@"; do
...
# Scan *.err and *_${JOB_PREFIX}*.err
ERR_FILES=("$ANALYSIS_DIR"/*_${JOB_PREFIX}*.err "$ANALYSIS_DIR"/*.err 2>/dev/null)
...
# Scan *.log and *_${JOB_PREFIX}*.log for failures
LOG_FILES=("$ANALYSIS_DIR"/*_${JOB_PREFIX}*.log "$ANALYSIS_DIR"/*.log 2>/dev/null)
...
for file in "${LOG_FILES[@]}" "$ANALYSIS_DIR"/*_${JOB_PREFIX}*.out "$ANALYSIS_DIR"/*.out 2>/dev/null; do
    if [[ -f "$file" ]]; then
        ...
        RECS=$(grep "Completed processing segment" "$file" | awk '{sum += $NF} END {print sum+0}')
        ...
        TIMES=$(grep -i "processing\|heartbeat" "$file" | awk '{if ($0 ~ /sec/) sum += $NF} END {print (sum/NR)+0}' || echo "0")
...
PARQUET_FILES=("$ANALYSIS_DIR"/crawldata*.parquet 2>/dev/null)
...
for file in parquet_files:
    if os.path.exists(file) and os.path.getsize(file) > 0:
        try:
            ...
            # Unique postcodes across all (flatten lists)
...
EOF
        ...
        # Write temp Python script and run
        TEMP_PY=$(mktemp)
        ...
        if [[ -x "$CONDA_ENV/bin/python" ]]; then
            ...
            rm "$TEMP_PY"
            echo "$STATS"
            TOTAL_ROWS=$(echo "$STATS" | grep "Total rows" | awk '{print $4}')
            UNIQUE_POSTCODES=$(echo "$STATS" | grep "Unique postcodes" | awk '{print $3}')
            QUALITY_NOTES=$(echo "$STATS" | grep "Avg % records")
            if [[ $with_postcodes_pct -lt 10 ]]; then  # Heuristic threshold
