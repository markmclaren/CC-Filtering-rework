...
def setup_logging(job_id=None):
    ...
    job_workdir = os.environ.get("WORKING_DIR", ".")

    # Include job ID in log filename if provided
    log_filename = (
        f"commoncrawl_processor_{job_id}.log" if job_id else "commoncrawl_processor.log"
    )
...
def Bristol_postcode_finder(text, postcode_lookup):
    ...
    matches = [
        postcode for postcode in postcodes if postcode in postcode_lookup["pcds"].values
    ]
    if matches:
        return matches
...
def process_segment(
    segment_number, wet_paths_file, server_url, output_dir, postcode_lookup=None
):
    ...
    try:
        ...
        if segment_number >= len(paths):
            logging.error(
                f"Segment number {segment_number} exceeds available paths ({len(paths)})"
            )
            return False
        ...
        records_processed = process_wet_file(
            gz_filepath, parquet_filepath, postcode_lookup
        )
        ...
        logging.info(
            f"Completed processing segment {segment_number}: {records_processed} records"
        )
        return True
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Crawl date identifier (e.g., 202350)",
    )
    parser.add_argument(
        "--server-url",
        type=str,
        default="https://data.commoncrawl.org/",
        help="CommonCrawl server URL",
    )
    parser.add_argument(
        "--wet-paths", type=str, default="wet.paths", help="Path to wet.paths file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of segments to process per task",
    )
    parser.add_argument(
        "--segment",
        type=int,
        help="Specific segment to process (for direct invocation)",
    )
    parser.add_argument(
        "--postcode-lookup",
        type=str,
        default="BristolPostcodeLookup.parquet",
        help="Path to postcode lookup file",
    )
    ...
    heartbeat_thread = threading.Thread(
        target=heartbeat, args=(logger, 300), daemon=True
    )
    ...
    if args.segment is not None:
        ...
        logger.info(
            f"Task {args.task_id} processing segments {start_segment} to {end_segment - 1}"
        )
    else:
    ...
    logger.info(
        f"Processing completed: {successful_segments}/{len(segment_numbers)} segments successful"
    )
