...
OUTPUT_SIZE=$(find "$WORK_DIR"/output* -name "*.parquet" 2>/dev/null | xargs stat -c%s 2>/dev/null | awk '{sum+=$1} END {print sum+0}')

echo "📊 Parquet output files: $OUTPUT_FILES"
echo "💾 Total parquet size: $(format_bytes $OUTPUT_SIZE)"

# SLURM logs
SLURM_OUT=$(ls "$WORK_DIR"/*.out 2>/dev/null | wc -l)
SLURM_ERR=$(ls "$WORK_DIR"/*.err 2>/dev/null | wc -l)

echo "📋 SLURM output logs: $SLURM_OUT"
echo "📋 SLURM error logs: $SLURM_ERR"

# Recent activity
RECENT_1H=$(find "$WORK_DIR"/output* -name "*.parquet" -newermt "1 hour ago" 2>/dev/null | wc -l)
RECENT_6H=$(find "$WORK_DIR"/output* -name "*.parquet" -newermt "6 hours ago" 2>/dev/null | wc -l)
...
for dir in "$WORK_DIR"/output*; do
    if [ -d "$dir" ]; then
        ...
        echo "📂 $dirname: $files files ($(format_bytes $size))"
    fi
done

# Check if no output directories found
if [ $(find "$WORK_DIR" -maxdepth 1 -name "output*" -type d 2>/dev/null | wc -l) -eq 0 ]; then
    echo "❌ No output* directories found in $WORK_DIR"
fi
...
echo "💿 STORAGE"
echo "----------------------------------------"
TOTAL_SIZE=$(du -sb "$WORK_DIR" 2>/dev/null | awk '{print $1}')
...
echo "🔗 QUICK ACTIONS"
echo "----------------------------------------"
echo "📊 Detailed analysis: ./file-analysis.sh"
echo "📈 Monitor real-time: ./monitor-disk-usage.sh"
echo "📋 Check job status: ./job-status.sh"
echo "🔍 View specific output: ls -la $WORK_DIR/output/"
